...
var Core = function(options) {
   ...
   var runSideMenu = function(options) {
      ...
      */
      ...
      $('.sidebar-menu li a.accordion-toggle .caret').on('click', function(e) {
         e.preventDefault();

         // If the clicked menu item is minified and is a submenu (has sub-nav parent) we do nothing
         if ($('body').hasClass('sb-l-m') && !$(this).parent().parents('ul.sub-nav').length) { return; }

         // If the clicked menu item is a dropdown we open its menu
         if (!$(this).parent().parents('ul.sub-nav').length) {
         ...
         // we only close menu items which are not a child of the uppermost top level menu
            ...
            var activeMenu = $(this).parent().next('ul.sub-nav');
            var siblingMenu = $(this).parent().parent().siblings('li').children('a.accordion-toggle.menu-open').next('ul.sub-nav')
         ...
         if (!$(this).parent().hasClass('menu-open')) {
            $(this).parent().next('ul').slideToggle('fast', 'swing', function() {
               $(this).attr('style', '').prev().toggleClass('menu-open');
            });
         }

      });
      // $('.sidebar-menu li a.firstDrop').on('dbclick', function() {
      //    var hrefNum = $(this).data('id');
      //    location.href('admin/product/category/' + hrefNum)
      // });
   }
