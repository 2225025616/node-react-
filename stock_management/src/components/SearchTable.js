...
class SearchFormItem extends PureComponent {
  render () {
    ...
    if (element === 'select') {
      return (
        <div {...props}>
          <Select {...props} style={{width}} placeholder="全部">
            {options.map(oItem => <Option key={oItem.value} value={oItem.value}>{oItem.label&&oItem.label||oItem.value}</Option>)}
          </Select>
        </div>
      )
...
class SearchForm extends Component {
  sendRequest = (value) => {
    ...
    if (value === -1) {
      ...
      // console.log('queryForm000',queryForm)
      search(parseInt(current), 10,queryForm)
    }
  }
  componentWillMount () {
